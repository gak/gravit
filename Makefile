...
BASEFILES = README COPYING Vera.ttf gravit.cfg demo.cfg screensaver.cfg ChangeLog particle.png gravit.png gravit.ico data/

CFLAGS = -O4 -Wall `sdl-config --cflags` -Wall
LDFLAGS = -L/usr/X11R6/lib -lGL -lGLU -lSDL_ttf -lSDL_image `sdl-config --libs`

DISTDIR = gravit-0.4.0
TMPBASE = /tmp
...
packagesrc:
	...
	mkdir $(TMPDIR)
	cp -r $(BASEFILES) *.c *.h Makefile gravit.dsp gravit.dsw $(TMPDIR)
	cd $(TMPDIR); chmod 644 *
	cd $(TMPBASE); tar -c --exclude=".svn" -zvf $(DISTDIR)-src.tgz $(DISTDIR)
...
packagewindll:
	...
	mkdir $(TMPDIR)
	cp -r $(BASEFILES) gravit.exe SDL.dll SDL_ttf.dll SDL_image.dll jpeg.dll libpng13.dll zlib1.dll $(TMPDIR)
	cd $(TMPDIR); sed -i 's/$$/\r/' README COPYING *.cfg ChangeLog
	cd $(TMPDIR); mv README README.txt
	cd $(TMPDIR); mv ChangeLog ChangeLog.txt
	cd $(TMPDIR); rm -r data/.svn
