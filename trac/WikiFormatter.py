...
class CommonFormatter:
    ...
    def _wikilink_formatter(self, match, fullmatch):
        if not self.env._wiki_pages.has_key(match):
            return '<a class="missing" href="%s">%s?</a>' % (self._href.wiki(match), match)
        else:
